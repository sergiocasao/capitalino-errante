...
if(!class_exists('AQ_Page_Builder')) {
	class AQ_Page_Builder {
		...
		function __construct( $config = array()) {
			...
			$defaults['menu_title'] = esc_attr__('Page Builder', 'aqpb-l10n');
			$defaults['page_title'] = esc_attr__('Page Builder', 'aqpb-l10n');
			$defaults['page_slug'] = esc_attr__('aqua-page-builder', 'aqpb-l10n');
		...
		function register_template_post_type() {
			...
			if(!post_type_exists('template')) {
				...
				add_action('admin_notices', create_function('', "echo '<div id=\"message\" class=\"error\"><p><strong>Aqua Page Builder notice: </strong>'. esc_attr__('The \"template\" post type already exists, possibly added by the theme or other plugins. Please consult with theme author to resolve this issue', 'aqpb-l10n') .'</p></div>';"));
			}
		...
		function display_blocks( $template_id ) {
			...
			if(empty($blocks)) {
				...
				echo esc_attr__('Drag block items from the left into this area to begin building your template.', 'aqpb-l10n');
				echo '</p>';
				return;
		...
		**/
		function update_template($template_id, $blocks, $title) {
			...
			if(!$this->is_template($template_id)) wp_die( esc_attr__('Error : Template id is not valid', 'aqpb-l10n') );
		...
		**/
		function display_template($template_id) {
			...
			if(empty($blocks)) {
				...
				echo '<p class="empty-template">';
				echo esc_attr__('This template is empty', 'aqpb-l10n');
				echo '</p>';
				
			} else {
				//template wrapper
				echo '<div id="aq-template-wrapper-'.esc_attr($template_id).'" class="aq-template-wrapper aq_row">';
		...
		function add_shortcode() {
			...
			if ( !array_key_exists( 'template', $shortcode_tags ) ) {
				...
				add_action('admin_notices', create_function('', "echo '<div id=\"message\" class=\"error\"><p><strong>Aqua Page Builder notice: </strong>'. esc_attr__('The \"[template]\" shortcode already exists, possibly added by the theme or other plugins. Please consult with the theme author to consult with this issue', 'aqpb-l10n') .'</p></div>';"));
			}
			...
			if ( in_array( $pagenow, array( 'post.php', 'page.php', 'post-new.php', 'post-edit.php' ) ) ) {
				...
				<div id="aqpb-iframe-container" style="display: none;">
					<div class="wrap" style="padding: 1em">
						...
						if ( empty( $templates ) ) {
							echo sprintf( esc_attr__( 'You don\'t have any template yet. Let\'s %s create %s one!', 'aqpb-l10n' ), '<a href="' .esc_url(admin_url()).'themes.php?page=aqua-page-builder">', '</a>' );
						} else { ?>						

							<h3><?php esc_attr_e( 'Choose Your Page Template', 'aqpb-l10n' ); ?></h3><br />
		...
		function contextual_help() {
			...
			if($screen->id == $this->page) {
				...
				$screen->set_help_sidebar(
					'<p><strong>' . esc_attr__('For more information:') . '</strong></p>' .
					'<p>' . esc_attr__('<a href="http://aquagraphite.com/api/documentation/aqua-page-builder" target="_blank">Documentation</a>') . '</p>' .
					'<p>' . esc_attr__('<a href="https://wordpress.org/plugins/aqua-page-builder/changelog/" target="_blank">Changelog</a>') . '</p>'
				);
				
				// Main overview tab
				$screen->add_help_tab( array(
				'id'		=> 'overview',
				'title'		=> esc_attr__('Overview'),
