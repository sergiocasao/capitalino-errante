...
if ( ! current_user_can('edit_theme_options') )
	wp_die( _e( 'Cheatin&#8217; uh?', 'jelly_text_main' ) );
...
switch($action) {
	...
	case 'create' :
		...
		if(!is_wp_error($new_id)) {
			...
			$messages[] = '<div id="message" class="updated"><p>' . esc_attr__('The ', 'aqpb-l10n') . '<strong>' . $template_name . '</strong>' . esc_attr__(' page template has been successfully created', 'aqpb-l10n') . '</p></div>';
		} else {
			$errors = '<ul>';
			...
			$messages[] = '<div id="message" class="error"><p>' . esc_attr__('Sorry, the operation was unsuccessful for the following reason(s): ', 'aqpb-l10n') . '</p>' . $errors . '</div>';
		}
	...
	case 'clone' :
		...
		if(!is_wp_error($new_id)) {
			...
			$messages[] = '<div id="message" class="updated"><p>' . esc_attr__('The ', 'aqpb-l10n') . '<strong>' . $template_name . '</strong>' . esc_attr__(' template has been successfully cloned', 'aqpb-l10n') . '</p></div>';
		} else {
			$errors = '<ul>';
			foreach( $new_id->get_error_messages() as $error ) {
				$errors .= '<li><strong>'. $error . '</strong></li>';
	...
	case 'update' :
		...
		$messages[] = '<div id="message" class="updated"><p>' . esc_attr__('The ', 'aqpb-l10n') . '<strong>' . $template_name . '</strong>' . esc_attr__(' page template has been updated', 'aqpb-l10n') . '</p></div>';
		break;
	...
	case 'delete' :
		...
		$messages[] = '<div id="message" class="updated"><p>' . esc_attr__('The template has been successfully deleted', 'aqpb-l10n') . '</p></div>';
		break;
}
...
<div class="wrap">
	...
	<div id="manage-templates" class="manage-templates">
		...
		<?php if($templates) { ?>
			...
			<form method="get" action="<?php echo esc_url(admin_url('themes.php')) ?>" >
				...
				<input type="hidden" name="page" value="<?php echo esc_attr($this->args['page_slug']) ?>">
				...
				<label for="template">
					...
					<select name="template" id="">
						...
						<?php if($selected_template_id === 0) { ?>
							<option value="0" selected="selected"><?php esc_attr_e('-- Select --', 'aqpb-l10n') ?></option>
						<?php } ?>
	...
	<div id="page-builder-frame">
		...
		<div id="page-builder-column" class="metabox-holder <?php echo esc_attr($disabled) ?>">
			<div id="page-builder-archive" class="postbox">
				<h3 class="hndle"><span><?php esc_attr_e('Available Blocks', 'aqpb-l10n') ?></span><span id="removing-block"><?php esc_attr_e('Deleting', 'aqpb-l10n') ?></span></h3>
				<div class="inside">
				...
				<div class="aqpb-wrap aqpbdiv">
					...
					<form id="update-page-template" action="<?php echo esc_url($this->args['page_url']) ?>" method="post" enctype="multipart/form-data">
						...
						<div id="aqpb-header">
								...
								<div id="submitpost" class="submitbox">
									<div class="major-publishing-actions cf">
										...
										<label class="open-label howto" for="template-name">
											<span><?php esc_attr_e('Template Name', 'aqpb-l10n') ?></span>
											<input name="template-name" id="template-name" type="text" class="template-name regular-text" title="Enter template name here" placeholder="<?php esc_attr_e('Enter template name here', 'aqpb-l10n') ?>" value="<?php echo is_object($selected_template_object) ? $selected_template_object->post_title : ''; ?>">
										</label>
										
										<div id="template-shortcode">
											<input type="text" readonly="readonly" value='[template id="<?php echo esc_attr($selected_template_id) ?>"]' onclick="select()"/>
										</div>
										
										<div class="publishing-action">
											<?php submit_button( empty( $selected_template_id ) ? esc_attr__( 'Create Template', 'aqpb-l10n' ) : esc_attr__( 'Save Template', 'aqpb-l10n' ), 'button-primary ', 'save_template', false, array( 'id' => 'save_template_header' ) ); ?>
								...
								<input type="hidden" name="template" id="template" value="<?php echo esc_attr($selected_template_id) ?>"/>
						...
						<div id="aqpb-body">
							...
							<ul class="blocks cf" id="blocks-to-edit">
								...
								if($selected_template_id === 0) {
									...
									echo esc_attr__('To create a custom page template, give it a name above and click Create Template. Then choose blocks like text, widgets or tabs &amp; toggles from the left column to add to this template.
							...
							<div class="major-publishing-actions cf">
								...
								<div class="delete-action">
									...
									echo '<a class="submitdelete deletion template-delete" href="' . esc_url(add_query_arg(
										...
										admin_url( 'themes.php' )
									)) . '">'. esc_attr__('Delete Template', 'aqpb-l10n') .'</a>';
									?>
                                ...
                                <div class="clone-action">
                                    ...
                                    echo '<a class="submitclone cloning template-clone" href="' . esc_url(add_query_arg(
                                        ...
                                        admin_url( 'themes.php' )
                                    )) . '">'. esc_attr__('Clone Template', 'aqpb-l10n') .'</a>';
                                    ?>
								...
								<div class="publishing-action">
									<?php if(!empty($selected_template_id)) {
										submit_button( esc_attr__( 'Save Template', 'jelly_text_main' ), 'button-primary ', 'save_template', false, array( 'id' => 'save_template_footer' ) ); 
									} ?>
								</div><!-- END .publishing-action -->
							</div><!-- END .major-publishing-actions -->
